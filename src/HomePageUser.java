...
public class HomePageUser extends JFrame {
    public HomePageUser(String username) {
        ...
        vehicleSearchPanel.setBorder(BorderFactory.createEmptyBorder(5, 30, 10, 55)); // Padding
        ...
        pricingTitle.setBorder(BorderFactory.createEmptyBorder(0,0,0,0));
        pricingPanel.add(pricingTitle);

        // Price Search Panel
        JPanel priceSearchPanel = new JPanel();
        priceSearchPanel.setLayout(new BoxLayout(priceSearchPanel, BoxLayout.X_AXIS));  // Ensure this is separate
        priceSearchPanel.setBackground(new Color(57, 62, 70));
        priceSearchPanel.setBorder(BorderFactory.createEmptyBorder(5, 30, 10, 55)); // Padding for width adjustment

        JLabel pricingSearchIcon = new JLabel(new ImageIcon("resources/yellow 50px/search.png")); // Replace with the actual icon path
        pricingSearchIcon.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 5));
        pricingSearchIcon.setAlignmentY(Component.CENTER_ALIGNMENT);

        JTextField priceSearchField = new JTextField("Search");
        priceSearchField.setFont(new Font("Poppins", Font.BOLD, 14));
        priceSearchField.setBackground(new Color(57,62,70));
        priceSearchField.setForeground(Color.GRAY);
        priceSearchField.setPreferredSize(new Dimension(400, 30));
        priceSearchField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 30)); // Stretchable width
        priceSearchField.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(255,211,105), 1, true),
                BorderFactory.createEmptyBorder(5, 10, 5, 10)));
        priceSearchPanel.add(pricingSearchIcon);
        priceSearchPanel.add(priceSearchField);


        // Add Focus Listener
        priceSearchField.addFocusListener(new FocusAdapter() {
            @Override
            public void focusGained(FocusEvent e) {
                if (priceSearchField.getText().equals("Search")) {
                    priceSearchField.setText("");
                    priceSearchField.setForeground(new Color(245,245,245));
                }
            }
            ...
            public void focusLost(FocusEvent e) {
                if (priceSearchField.getText().isEmpty()) {
                    priceSearchField.setText("Search");
                    priceSearchField.setForeground(Color.GRAY);
                }
            }
        });
        ...
        // Add Search Bar to Vehicle Panel
        pricingPanel.add(Box.createRigidArea(new Dimension(0, 0))); // Spacing below title
        pricingPanel.add(priceSearchPanel);
        ...
        // Create the TableRowSorter for the pricing table
        ...
        // Add the search filter functionality for the pricing table
        priceSearchField.getDocument().addDocumentListener(new DocumentListener() {
            @Override
            public void insertUpdate(DocumentEvent e) {
                filterPricingTable();
            }
            ...
            private void filterPricingTable() {
                String text = priceSearchField.getText();
                if (text.trim().isEmpty() || text.equals("Search")) {
                    sorterCarP.setRowFilter(null);
                } else {
                    sorterCarP.setRowFilter(RowFilter.regexFilter("(?i)" + text));
                }
            }
        });

        pricingPanel.addComponentListener(new ComponentAdapter() {
            @Override
            public void componentShown(ComponentEvent e) {
                // Refresh the pricing table data
                loadCarPricingData(pricingTable); // Reload data (implement this based on your data source)

                // Reapply the filter if a search term exists
                String searchText = priceSearchField.getText();
                if (!searchText.equals("Search") && !searchText.trim().isEmpty()) {
                    sorterCarP.setRowFilter(RowFilter.regexFilter("(?i)" + searchText));
                } else {
                    sorterCarP.setRowFilter(null); // Clear filter if search is empty
                }
            }
        });
        ...
        yourBookingsBtn.setFont(new Font("Poppins", Font.BOLD, 18));
        yourBookingsBtn.setForeground(new Color(57,62,70));
        yourBookingsBtn.setBackground(new Color(255,211,105));
        yourBookingsBtn.setFocusPainted(false);
        yourBookingsBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        yourBookingsBtn.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(34,40,49), 2, true),
                BorderFactory.createEmptyBorder(5, 10, 5, 10)));

        yourBookingsBtn.addActionListener(e -> {
            new YourBookings(username);
        });
        ...
        buttonPanel.add(yourBookingsBtn);
